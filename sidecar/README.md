...
**Feel free to just past this directly to your agent and ask for TL;DR. Hope you enjoy this as I did**

This repo is a set of March 2026 experiments in which a small external
...
Since Anthropic's July 2026 "global workspace" / J-lens
work formalizes the same underlying results on reading from and writing to a
model's internal state at inference time I felt empowered to share mine.
Anthropic: https://transformer-circuits.pub/2026/workspace/index.html

Difference between Anthropic and this experiments: The injected vector doesn't come
from a probe or adapter trained *on* the target model.
It comes from a **different, independently
...
- **Not a consciousness.** Nothing here demonstrates that a model is
  conscious, self-aware, or has subjective experience. I'd love to find that but
  is not here.
- **Not an isolated mechanism.** I didn't have time to go into the specific
  mechanism of why these experiment actually work. Most likely Anthropic study is
  the reason why.
- **It was a wild ride.** The findings below are a mix of a few working
...
logs before publication by Claude Code Opus 4.8, Codex 5.5, Grok 4.5, DeepSeek v4, GLM 5.2. Still me and them can be wrong but possibility is low.
...
**This table was made by Claude to avoid copy errors**

## How to replicate
This is a guide to replicate using your own corpus
...
3. **Bring your own corpus.** Nothing here reproduces the original PSN, because
   it was trained on my personal writing. See `data/corpus_format.md` for the
   ...
   **use take-outs from ChatGPT (recommended, one click) or Claude (slower)**
...
7. **(Optional side quest, not independently runnable) Read the PSN's internal state as
...
## What I tried to clean but still could create some confusion so consider this excluded:
...
- A parallel "Dark ASN" experiment line, built on a corpus of manipulative
  ...
  from the lab notes. **excluded to avoid triggering Fable or GPT 5.5 Cyber flags**
